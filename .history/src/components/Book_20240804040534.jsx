...
const Page = ({ number, front, back, page, opened, bookClosed, ...props }) => {
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < bones.length; i++) {
      ...
      const insideCurveIntensity = i < 8 ? Math.floor(Math.sin(i * 0.2 + 0.25)) : 0;
      const outsideCurveIntensity = i >= 8 ? Math.cos(i * 0.3 + 0.09) : 0;
      let rotationAngle = insideCurveStrength * insideCurveIntensity * targetRotation - outsideCurveIntensity * outsideCurveStrength * targetRotation;
  ...
  return (
    <group {...props} ref={pageRef} >
      <primitive
        object={manualSkinnedMesh}
        ref={skinnedMeshRef}
        position-z={-number * PAGE_DEPTH + (page > number ? 1 : -1) * PAGE_DEPTH}
      />
    </group>
  );
};
